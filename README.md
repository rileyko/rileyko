<h1 align="center">고우림 · Ko WooLim</h1>

<p align="center">
  <strong>데이터로 고객과 시장을 이해하고, 성장의 다음 액션을 설계하는 예비 그로스 마케터</strong>
</p>

<p align="center">
  <a href="https://vistawoo.com"><img src="https://img.shields.io/badge/Website-vistawoo.com-111111?style=flat-square&logo=googlechrome&logoColor=white" alt="Website" /></a>
  <a href="https://www.linkedin.com/in/woolim-ko-205917195/"><img src="https://img.shields.io/badge/LinkedIn-Ko%20WooLim-0A66C2?style=flat-square&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="https://public.tableau.com/app/profile/riley5917/vizzes"><img src="https://img.shields.io/badge/Tableau%20Public-Data%20Viz-E97627?style=flat-square&logo=tableau&logoColor=white" alt="Tableau Public" /></a>
</p>

<br />

## 👋 안녕하세요, 고우림입니다

데이터를 분석해 **사용자와 시장의 맥락을 이해하고**, 이를 실제 성장 아이디어와 실행으로 연결하는 마케터를 준비하고 있습니다.  
데이터 전문가·AI/ML·개발 경험을 바탕으로, 지금은 마케팅 분석·SQL·GA4·Tableau·그로스 전략을 차근차근 쌓아가고 있어요.

> **Data → Insight → Action → Growth**  
> 숫자 뒤에 있는 고객의 이야기를 찾고, 더 나은 다음 행동을 제안합니다.

## 🌱 지금 집중하고 있어요

- 데이터 기반 그로스 마케팅과 퍼포먼스 분석
- 마케팅 지표 설계, SQL, GA4, Tableau 시각화
- Python을 활용한 데이터 분석과 실험 아이디어 도출
- 고객·시장 리서치를 바탕으로 한 문제 정의와 기획

## 🗂️ 대표 프로젝트

| Project | Summary | Skills |
| :-- | :-- | :-- |
| [Growth Marketing Bootcamp](https://github.com/rileyko/growth-marketing-bootcamp) | 데이터 기반 그로스 마케팅 부트캠프 포트폴리오 | Growth Marketing · Analysis |
| [Marketing Data Analysis](https://github.com/rileyko/marketing-data-analysis) | 마케터를 위한 Python 기반 데이터 분석 교육 자료 | Python · Pandas · Jupyter |
| [Cooling Shampoo Analysis](https://github.com/rileyko/cooling-shampoo-analysis) | 올리브영 리뷰 데이터로 시장을 분석하고 신제품을 기획한 프로젝트 | Research · Insight · Planning |

<p align="right"><a href="https://public.tableau.com/app/profile/riley5917/vizzes">→ Tableau Public에서 데이터 시각화 보기</a></p>

## 🧰 Tools I use

<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" alt="Python" />
  <img src="https://img.shields.io/badge/SQL-336791?style=flat-square&logo=postgresql&logoColor=white" alt="SQL" />
  <img src="https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white" alt="Tableau" />
  <img src="https://img.shields.io/badge/Google%20Analytics%204-E37400?style=flat-square&logo=googleanalytics&logoColor=white" alt="GA4" />
  <img src="https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white" alt="Pandas" />
  <img src="https://img.shields.io/badge/Salesforce-00A1E0?style=flat-square&logo=salesforce&logoColor=white" alt="Salesforce" />
</p>

## 💼 Experience

| 기간 | 경험 | 주요 업무 |
| :-- | :-- | :-- |
| 2023.03 – 2024.12 | **트레슬 · 개발팀 / 데이터 전문가** | 데이터 활용 전반, 운영 개선, 사용자 지원 · NetZero 대시보드 고도화 · 알레르망 PoC · K2 유지보수 |
| 2022.12 – 2023.03 | **메가존클라우드 · AIML / 세일즈 매니저 (계약직)** | AWS AI/ML PoC와 컨설팅 · 라스트오더 PoC |
| 2021.04 – 2022.08 | **아이투맥스 · R&D / 연구원 (데이터 파트장)** | SFDC 데이터 분석·시각화·플랫폼 · 제네시스유럽 CXP · 현대자동차 DMS · 삼성 반도체 PoC |
| 2020.12 – 2021.03 | **코스켄 · AI사업본부 / 풀스택 개발자 (인턴)** | Python 기반 AI 큐레이터 웹 서비스 풀스택 개발 |

---

<p align="center">
  <a href="https://vistawoo.com">vistawoo.com</a> ·
  <a href="https://www.linkedin.com/in/woolim-ko-205917195/">LinkedIn</a> ·
  <a href="https://public.tableau.com/app/profile/riley5917/vizzes">Tableau Public</a>
</p>

<p align="center"><sub>데이터를 더 좋은 고객 경험과 성장으로 연결합니다 ✨</sub></p>
